...
export function calculateLSystem(initial, rules, iterations) {
  if (iterations <= 0) {
    return initial;
  } else {
    ...
    var regex = new RegExp(
      Object.keys(rulesHash).join("|").replace("+", "\\+"),
      "gi"
    );

    // Calculate the next iteration
    var nextIteration = initial.replace(regex, function (matched) {
      return rulesHash[matched] || "";
    });
...
export function traceLSystem(steps, angle) {
  ...
  return {
    positions: zeroAdjustedPositions,
    width: boundary.max.x - boundary.min.x,
    height: boundary.max.y - boundary.min.y,
  };
}

// export default { calculateLSystem, traceLSystem };
