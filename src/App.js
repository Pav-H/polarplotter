...
import { calculateLSystem, traceLSystem } from "./l-systems";
  ...
  sampleRules() {
    return {
      ...
      Fern: {
        ...
        angle: 15,
        rules:
          "A: C[+++++F]C[-----F]C[++++DG]C[------DG]-A\nB: BC\nC: B\nD: C\nF: A\nG: F",
      },
      Antenna: {
        initial: "++A",
        angle: 18,
        rules:
          "A: C[C+++++F]C[-----F]C[C+++++DG]C[-----DG]A\nB: BC\nC: B\nD: C\nF: A\nG: F",
      },
    };
  }
  ...
  drawLSystem() {
    ...
    for (var i = 0; i < tracing.positions.length; i++) {
      if (tracing.positions[i] != null) {
        var char = tracing.positions[i - 1] == null ? "M" : "L";
          ...
          <div className="four columns">
            ...
            >
              <p>
                ...
                are a type of formal grammar which uses recursively-applied
                character substitution rules to produce strings that can then be
                translated into fractal images via turtle graphics. Each
                character in the output string corresponds to an instruction
                like "turn right," "turn left," or "move forward."
              </p>
              <p>
                In the widget below, the characters{" "}
                <span style={{ fontFamily: "monospace" }}>+</span> and{" "}
                <span style={{ fontFamily: "monospace" }}>-</span> rotate the
                turtle left and right. The letters{" "}
                <span style={{ fontFamily: "monospace" }}>A-M</span> move the
                turtle forward one step, and the letters{" "}
                <span style={{ fontFamily: "monospace" }}>N-Z</span> are
                placeholders which don't produce any action, but control the
                outcome's evolution.{" "}
                <span style={{ fontFamily: "monospace" }}>[</span> and{" "}
                <span style={{ fontFamily: "monospace" }}>]</span> are special
                symbols, which push to or pop the turtle's position from an
                array, causing the turtle to jump from one position to another.
              </p>
            </div>
            ...
            >
              <label>
                Select an example L-System or build a custom one below
              </label>
